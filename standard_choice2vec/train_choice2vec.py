...
def train_choice2vec(num_epochs: int = 20, 
    ...
    trainer = Choice2VecTrainer(
        ...
        use_wav2vec2_loss=use_wav2vec2_loss,
...
def visualize_results(training_history, contextualized_features, states, quantized_indices, 
    ...
    if save_plots:
        plt.savefig('results/choice2vec_training_results.png', dpi=300, bbox_inches='tight')
        print(f"\n📊 Results saved as 'results/choice2vec_training_results.png'")
...
def main():
    ...
    # Train model with wav2vec 2.0 InfoNCE loss
    print("\n🏋️ Training Choice2Vec model with wav2vec 2.0 InfoNCE loss...")
    state, data, training_history, trainer = train_choice2vec(
        num_epochs=1000,
        batch_size=16,  # Increased batch size for GPU efficiency
        learning_rate=1e-4,
        use_cosine_loss=False,  # Disable simple cosine loss
        use_wav2vec2_loss=True,  # Enable wav2vec 2.0 InfoNCE loss
        temperature=0.1,  # Temperature parameter κ from wav2vec 2.0 paper
        num_negatives=100,  # Number of negative samples K
        verbose=True
    )
    ...
    print(f"\n📊 Model parameters saved as 'results/{model_name}.pkl'")
